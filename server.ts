...
# Pydantic Schemas matching Frontend TypeScript Contracts
...
class ReportRequest(BaseModel):
    stationId: str
    fuelType: Optional[str] = None # "AI92", "AI95", "DT"
    type: Optional[str] = None # "available", "empty", "unloading"
    queueLength: Optional[str] = None # "none", "short", "medium", "long"
    source: str = "report_app"
    sender: Optional[str] = "Водитель"
...
class ChatMessageResponse(BaseModel):
    ...
    sender: str
    text: str
    parsed: Optional[ParsedMessageSchema] = None
    source: str = "telegram"
    isFake: bool = False
    createdAt: str

# Helper to serialize DB Station to Frontend Station format
def map_db_station(s: GasStationDB) -> StationResponse:
    return StationResponse(
        id=str(s.id),
        brand=s.brand,
        address=s.address,
        coords=CoordsSchema(lat=s.latitude, lng=s.longitude),
        status=FuelStatusSchema(
            AI92=s.AI92_status or "available",
            AI95=s.AI95_status or "available",
            DT=s.DT_status or "available"
        ),
        queueLength=s.queue_length or "none",
        waitingTime=s.waiting_time or 0,
        confidenceScore=s.confidence_score or 0.8,
        deficitForecast=s.deficit_forecast or "low",
        nextDelivery=s.next_delivery or "нет данных",
        updatedAt=s.updated_at.isoformat() + "Z" if s.updated_at else datetime.utcnow().isoformat() + "Z",
        apiUrl=s.api_url,
        apiKey=s.api_key
    )

# Helper to serialize DB Chat Message to Frontend format
def map_chat_message(m: ChatMessageDB) -> ChatMessageResponse:
    parsed_data = None
    if m.parsed_json:
        try:
            parsed_dict = json.loads(m.parsed_json)
            parsed_data = ParsedMessageSchema(
                isFuelRelated=parsed_dict.get("is_fuel_related", False),
                brand=parsed_dict.get("brand"),
                address=parsed_dict.get("address"),
                fuelType=parsed_dict.get("fuelType"),
                status=parsed_dict.get("status"),
                queue=parsed_dict.get("queue"),
                isFake=parsed_dict.get("is_fake", False),
                explanation=parsed_dict.get("explanation", "Успешно разобрано NLP")
            )
        except Exception:
            pass
...
def get_stations(db: Session = Depends(get_db)):
    db_stations = db.query(GasStationDB).all()
    return [map_db_station(s) for s in db_stations]

@app.post("/api/reports")
def submit_direct_report(req: ReportRequest, db: Session = Depends(get_db)):
    station = db.query(GasStationDB).filter(GasStationDB.id == int(req.stationId)).first()
    if not station:
        raise HTTPException(status_code=404, detail="Station not found")
    
    if req.fuelType and req.type:
        if req.fuelType == "AI92": station.AI92_status = req.type
        elif req.fuelType == "AI95": station.AI95_status = req.type
        elif req.fuelType == "DT": station.DT_status = req.type

    if req.queueLength:
        station.queue_length = req.queueLength
        if req.queueLength == "none": station.waiting_time = 0
        elif req.queueLength == "short": station.waiting_time = 10
        elif req.queueLength == "medium": station.waiting_time = 25
        elif req.queueLength == "long": station.waiting_time = 50

    station.confidence_score = min(1.0, station.confidence_score + 0.05)
    station.updated_at = datetime.utcnow()
    
    # Save a chat message log
    msg_text = f"[Отчет] АЗС {station.brand} ({station.address}): {req.fuelType or ''} - {req.type or ''}, очередь: {req.queueLength or ''}"
    new_msg = ChatMessageDB(
        sender=req.sender or f"Система ({req.source})",
        text=msg_text,
        is_fake=False,
    ...
    return {"status": "ok", "station": map_db_station(station)}
    ...
    if is_related and not is_fake:
        ...
        if brand:
            ...
            if station:
                ...
                db.commit()
                db.refresh(station)
                stations_updated.append(map_db_station(station))

    if not stations_updated:
        # Default: return current list of all stations
        stations_updated = [map_db_station(s) for s in db.query(GasStationDB).all()]

    return {
        "status": "ok", 
        "message": map_chat_message(new_msg),
        "stations": stations_updated
    }

@app.get("/api/chat/latest", response_model=List[ChatMessageResponse])
def get_latest_chats(db: Session = Depends(get_db)):
    db_messages = db.query(ChatMessageDB).order_by(ChatMessageDB.created_at.desc()).limit(20).all()
    return [map_chat_message(m) for m in db_messages]
...
class GasStationDB(Base):
    ...
    # Fuel statuses
    AI92_status = Column(String, default="available") # available / empty / unloading / unknown / coupon
